...
class OrdersDatabase {
  ...
  static Future<void> getData(Database database, int userId) async {
    ...
    await database.rawQuery(
        'SELECT * FROM orders where userId = ? ', [userId]).then((value) {
  ...
  static Future<void> getDataForBestSeller(Database database) async {
    ...
    await database.rawQuery('SELECT * FROM orders ').then((value) {
      value.forEach((element) {
        ...
        if (bestSellerData[productId] == null) {
          bestSellerData[productId] = 0;
        }
        int val = int.parse(bestSellerData[productId].toString());
        bestSellerData[productId] =
            val + int.parse(element["quantity"].toString());
      });
      getBest7();
      mp.forEach((key, value) {
        orderIds.add(key);
  ...
  static void deleteDatabase() async {
    await database.rawDelete('DELETE FROM orders').then((value) {
      print(value);
    }).catchError((error) {
      print(error.toString());
    });
